...
from miband_api.constants import UUIDS, AUTH_STATES, ALERT_TYPES, QUEUE_TYPES, MUSICSTATE
import struct
...
class miband(Peripheral):
    ...
    def loadHeartRate(self):
        ...
        #t = time.time()
        self.waitForNotifications(0.5)
        hRValue = self._parse_queue2()
        return hRValue
            # send ping request every 12 sec

    def sendPingHeartRate(self):
        char_ctrl = self.svc_heart.getCharacteristics(UUIDS.CHARACTERISTIC_HEART_RATE_CONTROL)[0]
        char_ctrl.write(b'\x16', True)
